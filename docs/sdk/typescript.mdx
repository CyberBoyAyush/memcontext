...
Saves durable memory. MemContext automatically handles embeddings, deduplication, relationship classification, and versioning. Larger notes may be accepted for background extraction into atomic memories.
...
When `status` is `accepted`, the response includes a `jobId` and message. This means the note was stored and will be split into atomic memories in the background.
