...
For docs sites that expose `llms.txt`, MemContext discovers the canonical page
map first and ingests only the top-priority pages. `priorityPageLimit` defaults
to `15` and is capped at `25`.
