...
| `scope`      | string | No       | Hard isolation boundary. Use a stable end-user or tenant ID like `user_123` or `org_acme_user_42`. When provided, MemContext saves and retrieves only within that scope.           |
| `project`    | string | No       | Project grouping inside the selected scope. Use only when a clear project/app name is already known. Examples: `memcontext`, `carq`. Omit if unsure. Format: lowercase, no spaces. |
