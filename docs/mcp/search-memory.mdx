...
| `scope`     | string | No       | global  | Hard isolation boundary. When provided, search runs only in that scope. Omit to search only unscoped/global memories. |
| `project`   | string | No       | all     | Filter to a specific project inside the selected scope.                                                               |
