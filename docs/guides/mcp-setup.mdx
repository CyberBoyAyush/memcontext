...
- Important app-user or tenant context learned → save_memory(scope: "<stable-user-or-tenant-id>")
- Significant work completed → save_memory(category: "context")
- Use scope when one MemContext API key serves multiple app users or tenants. Scope is the hard isolation boundary.
