...
For larger notes, `POST /api/memories` may return `202 Accepted`. This means the note has been stored and will be extracted into atomic memories in the background.
...
- `accepted` → source stored, background extraction in progress
