...
Longer notes may return `202 Accepted` with a `jobId` and message. In that case, MemContext stores the source note immediately and extracts atomic memories in the background.
